...
class Host(models.Model):
    ...
    asset = models.OneToOneField('asset.Asset', verbose_name="資產編號", null=True, blank=True, related_name='Host',on_delete=models.CASCADE)
    ops_owner = models.ForeignKey('asset.UserProfile', verbose_name="運維負責人", null=True, blank=True,on_delete=models.CASCADE)
    location = models.ForeignKey('asset.Location', verbose_name='位置', null=True, blank=True,on_delete=models.CASCADE)
...
class NetworkDevice(models.Model):
    ...
    asset = models.OneToOneField('asset.Asset',on_delete=models.CASCADE)
    type_choices = (
        ('路由器', '路由器'),
        ('交換器', '交換器'),
        ('AP', 'AP'),
...
class HostRecord(models.Model):
    ...
    主機變更紀錄表
    """
    host_obj = models.ForeignKey('Host',on_delete=models.CASCADE)
    title = models.CharField(max_length=255)
    summary = models.TextField(null=True, blank=True)
    creator = models.ForeignKey('asset.UserProfile', verbose_name='創建者', null=True, blank=True, related_name='+',on_delete=models.CASCADE)
